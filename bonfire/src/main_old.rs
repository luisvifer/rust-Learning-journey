...
fn fill_bottom_with_random_ashes(fire_buf: &mut[u8], rng: &mut Wyrand) {
    ...
    let start = end - WINDOWS_WIDTH as usize;
    for i in start..end {
        fire_buf[i] = rng.next_range(0..PALETTE_SIZE as u16) as u8;
    }
}
...
fn calculate_next_fire_frame(fire_buf: &mut[u8]) {
    ...
    for y in 0..WINDOW_HEIGHT-1 {
        for x in 1..WINDOWS_WIDTH -1 {
            let i = (y*WINDOWS_WIDTH + x) as usize;

            fire_buf[i] = ((
                    10 * old_fire_buf[i - 1] as u64
                +   20 * old_fire_buf[i + 0] as u64
                +   10 * old_fire_buf[i + 1] as u64
                +  160 * old_fire_buf[i - 1 + WINDOWS_WIDTH as usize] as u64
                +  320 * old_fire_buf[i + 0 + WINDOWS_WIDTH as usize] as u64
                +  160 * old_fire_buf[i + 1 + WINDOWS_WIDTH as usize] as u64
            ) / 680) as u8;
        }
    }
